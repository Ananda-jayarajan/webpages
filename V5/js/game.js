...
/* -------------------- comment -------------------- */
console.log("Thalapathy Run: auto-run story mode loaded");
...
function updatePlayer(dt) {
  // AUTO-RUN MODE:
  // Vijay always runs forward. The player no longer controls left/right movement.
  // Controls are now: Jump, Slide, Pause, Music, and Quiz testing.
  const targetSpeed = CONFIG.physics.autoRunSpeed || 360;
  const acceleration = CONFIG.physics.autoRunAcceleration || CONFIG.physics.moveAcceleration || 2600;

  player.facing = 1;

  if (player.vx < targetSpeed) {
    player.vx += acceleration * dt;
    player.vx = Math.min(player.vx, targetSpeed);
  } else if (player.vx > targetSpeed) {
    player.vx -= acceleration * dt;
    player.vx = Math.max(player.vx, targetSpeed);
  }

  // Optional tiny slide slowdown so sliding feels physical, but the run never stops.
  if (keys.down && player.grounded) {
    player.vx = Math.max(targetSpeed * 0.82, player.vx * 0.985);
    player.height = player.normalHeight;
  } else {
    player.height = player.normalHeight;
  }
...
function initGame() {
  ...
  // Left/right buttons are intentionally unused in auto-run mode.
  ...
  document.addEventListener("keydown", event => {
    ...
    if (event.code === "ArrowLeft" || event.code === "KeyA" || event.code === "ArrowRight" || event.code === "KeyD") {
      event.preventDefault();
      // Auto-run mode: horizontal movement is locked forward.
      return;
    }
  ...
  console.log("Thalapathy Run auto-run ready. Press Space/W/Up to jump, S/Down to slide, Q to test quiz.");
}
