...
function drawPlayer() {
  ...
  drawCharacterShadow(x, groundY);

  const bob = player.grounded && !player.sliding ? Math.sin(t * Math.PI * 2) * 2 : 0;

  drawLeg(x + 18, y + 82 + bob, Math.sin(t) * 0.65);
  drawLeg(x + 43, y + 82 - bob, Math.sin(t + Math.PI) * 0.65);

  drawArm(x + 8, y + 42 + bob, Math.sin(t + Math.PI) * 0.65);
  drawArm(x + 55, y + 42 - bob, Math.sin(t) * 0.65);

  drawSuit(x + 6, y + 35 + bob);
  drawScarf(x + 10, y + 43 + bob);
  drawHead(x + 14, y + 1 + bob);
...
function drawCharacterShadow(x, y) {
  ...
  ctx.fillStyle = "#000";
  ctx.beginPath();
  ctx.ellipse(x + 34, y + 8, player.sliding ? 50 : 36, 9, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.restore();
}
