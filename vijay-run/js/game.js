...
assets.player.onerror = () => {
  ...
  console.warn("Could not load player image. Expected file:", CONFIG.player.image);
  render();
};
...
function setupAudio() {
  ...
  bgMusic.addEventListener("error", () => {
    ...
    console.warn("Could not load background music. Expected file:", CONFIG.audio.src);
    updateMusicButton();
  });
...
function startMusic() {
  if (!CONFIG.audio.enabled || !musicEnabled || musicLoadFailed) return;
...
function toggleMusic() {
  if (!CONFIG.audio.enabled || musicLoadFailed) return;
...
function updateSpawns(dt) {
  ...
  if (nextObstacleIn <= 0) {
    ...
    const base = CONFIG.difficulty.obstacleBaseGap;
    const random = CONFIG.difficulty.obstacleRandomGap;
    const gapPixels = base + Math.random() * random;
...
function spawnObstacle() {
  ...
  const y = isAir
    ? groundY - player.normalHeight + 20
...
function drawMissingImageNotice() {
  if (!playerImageFailed) return;
  ...
  ctx.lineWidth = 2;
  ctx.beginPath();
  ctx.roundRect(18, height - 118, Math.min(510, width - 36), 72, 16);
  ctx.fill();
  ctx.stroke();

  ctx.fillStyle = "#fff";
  ctx.font = "900 15px Arial";
  ctx.fillText("Missing player image:", 36, height - 88);

  ctx.fillStyle = "#ffd76a";
  ctx.font = "800 14px Arial";
  ctx.fillText("Put vijay.png inside assets/characters/", 36, height - 62);
  ctx.restore();
}
...
function drawCharacterShadow(x, y) {
  ...
  ctx.beginPath();
  ctx.ellipse(
    x + player.normalWidth / 2,
    y + 8,
    player.sliding ? 58 : 46,
    10,
    0,
    0,
    Math.PI * 2
...
function drawFallbackPlayer() {
  ...
  const bob = player.grounded && !player.sliding ? Math.sin(t * Math.PI * 2) * 2 : 0;

  drawLeg(x + 30, y + 82 + bob, Math.sin(t) * 0.65);
  drawLeg(x + 58, y + 82 - bob, Math.sin(t + Math.PI) * 0.65);

  drawArm(x + 20, y + 42 + bob, Math.sin(t + Math.PI) * 0.65);
  drawArm(x + 70, y + 42 - bob, Math.sin(t) * 0.65);

  drawSuit(x + 22, y + 35 + bob);
  drawScarf(x + 26, y + 43 + bob);
  drawHead(x + 28, y + 1 + bob);
...
function drawHead(x, y) {
  ...
  ctx.beginPath();
  ctx.roundRect(x + 11, y + 8, 38, 42, 18);
  ctx.fill();
  ctx.stroke();

  ctx.fillStyle = "#111";
  ctx.beginPath();
  ctx.roundRect(x + 3, y + 2, 55, 25, 14);
  ctx.fill();

  ctx.fillStyle = "#111";
  ctx.beginPath();
  ctx.roundRect(x + 17, y + 24, 14, 10, 4);
  ctx.roundRect(x + 33, y + 24, 14, 10, 4);
  ctx.fill();

  ctx.strokeStyle = "#111";
  ctx.lineWidth = 2;
  ctx.beginPath();
  ctx.moveTo(x + 31, y + 29);
  ctx.lineTo(x + 34, y + 29);
  ctx.stroke();

  ctx.fillStyle = "#111";
  ctx.beginPath();
  ctx.ellipse(x + 31, y + 42, 12, 4, 0, 0, Math.PI * 2);
  ctx.fill();
}
...
function drawSuit(x, y) {
  ...
  ctx.beginPath();
  ctx.roundRect(x + 25, y + 11, 6, 36, 3);
  ctx.fill();
}

function drawScarf(x, y) {
  ctx.save();
  ctx.fillStyle = CONFIG.colors.scarf;
  ctx.strokeStyle = "#111";
  ctx.lineWidth = 2;

  ctx.beginPath();
  ctx.roundRect(x - 2, y + 3, 60, 14, 8);
  ctx.fill();
  ctx.stroke();

  ctx.fillStyle = "#c92525";
  ctx.beginPath();
  ctx.roundRect(x + 42, y + 10, 15, 47, 6);
  ctx.fill();
  ctx.stroke();

  ctx.fillStyle = "#111";
  ctx.font = "900 9px Arial";
  ctx.fillText("TVK", x + 17, y + 14);
  ctx.restore();
}

function drawArm(x, y, swing) {
  ctx.save();
  ctx.translate(x, y);
  ctx.rotate(swing);
  ctx.fillStyle = CONFIG.colors.suit;
...
function drawLeg(x, y, swing) {
  ...
  ctx.lineWidth = 3;
  ctx.beginPath();
  ctx.roundRect(-8, 0, 18, 40, 7);
  ctx.fill();
  ctx.stroke();

  ctx.fillStyle = "#111";
  ctx.beginPath();
  ctx.roundRect(-11, 34, 24, 9, 5);
  ctx.fill();
  ctx.restore();
}
...
function drawObstacles() {
  for (const obstacle of obstacles) {
    ...
    if (obstacle.type === "air") {
      ctx.beginPath();
      ctx.roundRect(0, 0, obstacle.width, obstacle.height, 16);
      ctx.fill();
      ctx.stroke();
